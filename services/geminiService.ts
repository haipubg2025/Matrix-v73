...
import { GameUpdate, GameGenre, Relationship, Player, AppSettings, AiModel, ThinkingLevel, ResponseLength, WritingStyle, NarrativePerspective, GameTime, CodexEntry } from "../types";
...
export class GeminiService {
  ...
  private cleanAiContent(text: string): string {
    if (!text) return "";
    
    // Áp dụng các script regex tùy chỉnh (bao gồm cả script xóa thinking tags mới)
    let cleaned = regexService.applyScripts(text);

    // Các biện pháp làm sạch bổ sung để đảm bảo tính triệt để
    cleaned = cleaned.replace(/<thinking>[\s\S]*?<\/thinking>/gi, '');
    cleaned = cleaned.replace(/\[thinking\][\s\S]*?\[\/thinking\]/gi, '');
    cleaned = cleaned.replace(/<reasoning>[\s\S]*?<\/reasoning>/gi, '');
    cleaned = cleaned.replace(/\[reasoning\][\s\S]*?\[\/reasoning\]/gi, '');
    cleaned = cleaned.replace(/<word_count>[\s\S]*?<\/word_count>/gi, '');
    cleaned = cleaned.replace(/\[word_count\][\s\S]*?\[\/word_count\]/gi, '');
    cleaned = cleaned.replace(/<thinking>/gi, '').replace(/<\/thinking>/gi, '');
    cleaned = cleaned.replace(/<reasoning>/gi, '').replace(/<\/reasoning>/gi, '');
    cleaned = cleaned.replace(/<word_count>/gi, '').replace(/<\/word_count>/gi, '');
  ...
  private tryFixJson(jsonStr: string): string {
    ...
    // 2. Remove any leading/trailing non-JSON content (like markdown text)
    ...
    // 3. Handle truncation: If it doesn't end with }, try to close it
    if (!fixed.endsWith("}")) {
      // Close open quotes first
      const quoteCount = (fixed.match(/"/g) || []).length;
      if (quoteCount % 2 !== 0) fixed += '"';

      // Basic attempt to close open structures
  ...
  private normalizeGameUpdate(data: any): GameUpdate {
    ...
    if (data.text) data.text = this.cleanAiContent(data.text);
    if (data.summary) data.summary = this.cleanAiContent(data.summary);
...
${TIME_LOGIC_RULES}
    ...
    try {
      const fixedJson = this.tryFixJson(response.text || "{}");
      const result = JSON.parse(fixedJson);
      return this.normalizeGameUpdate(result);
    } catch (e) {
      console.warn("JSON Parse failed, attempting Tag-based extraction fallback...");
      try {
        // Fallback: Extract fields using regex tags if JSON is malformed
        const text = this.extractTag(response.text || "", "text");
        const summary = this.extractTag(response.text || "", "summary");
        const statsStr = this.extractTag(response.text || "", "statsUpdates");
        const statsUpdates = statsStr ? JSON.parse(this.tryFixJson(statsStr)) : {};
  ...
  private extractTag(content: string, tag: string): string {
    // Try to find content between "tag": "..." or <tag>...</tag>
    // 1. Try JSON-like property extraction (very loose)
    const jsonRegex = new RegExp(`"${tag}"\\s*:\\s*"(.*)"`, 's');
    const jsonMatch = content.match(jsonRegex);
    if (jsonMatch && jsonMatch[1]) return jsonMatch[1];

    // 2. Try XML-like tag extraction
...
export class GeminiGameService {
  ...
  private safeMapJoin(val: any, mapFn: (item: any) => string, separator: string = ','): string | undefined {
  ...
  private cleanAiContent(text: string): string {
    if (!text) return "";
    // Xóa tất cả các cặp thẻ đã đóng hoàn chỉnh (cả < > và [ ])
    let cleaned = text.replace(/<word_count>[\s\S]*?<\/word_count>/gi, '');
    cleaned = cleaned.replace(/\[word_count\][\s\S]*?\[\/word_count\]/gi, '');
    
    // Xóa các thẻ suy luận (thinking/reasoning) phổ biến
    cleaned = cleaned.replace(/<thinking>[\s\S]*?<\/thinking>/gi, '');
    cleaned = cleaned.replace(/\[thinking\][\s\S]*?\[\/thinking\]/gi, '');
    cleaned = cleaned.replace(/<reasoning>[\s\S]*?<\/reasoning>/gi, '');
    cleaned = cleaned.replace(/\[reasoning\][\s\S]*?\[\/reasoning\]/gi, '');
    
    // Xóa cả các thẻ mở hoặc đóng lẻ loi nếu còn sót lại
    cleaned = cleaned.replace(/<word_count>/gi, '').replace(/<\/word_count>/gi, '');
    cleaned = cleaned.replace(/\[word_count\]/gi, '').replace(/\[\/word_count\]/gi, '');
    cleaned = cleaned.replace(/<thinking>/gi, '').replace(/<\/thinking>/gi, '');
    cleaned = cleaned.replace(/<reasoning>/gi, '').replace(/<\/reasoning>/gi, '');
    
    // Xóa các nội dung rác thường gặp từ AI
    cleaned = cleaned.replace(/Tiến độ:.*?\n/gi, '');
  ...
  private extractTextFromPartialJson(partialText: string): string {
    // Nếu là JSON (bắt đầu bằng { hoặc có dấu hiệu "text": ")
    const isJson = partialText.trim().startsWith('{') || partialText.includes('"text":');
    ...
    if (isJson) {
      ...
      // Nếu là text thuần (đã tắt JSON mode)
      // Tìm vị trí bắt đầu của text (sau các thẻ word_count, thinking)
      // Tìm vị trí kết thúc của text (trước khi bắt đầu JSON block)
      
      let cleaned = partialText;
      // Xóa các thẻ hệ thống để lấy text sạch
      cleaned = cleaned.replace(/<word_count>[\s\S]*?<\/word_count>/gi, '');
      cleaned = cleaned.replace(/<thinking>[\s\S]*?<\/thinking>/gi, '');
      cleaned = cleaned.replace(/<reasoning>[\s\S]*?<\/reasoning>/gi, '');
      
      // Nếu AI đang viết JSON ở cuối, ta cắt bỏ phần đó
      const jsonStartIndex = cleaned.lastIndexOf('{');
      if (jsonStartIndex !== -1 && cleaned.includes('"summary"')) {
...
`;
  ...
  private async getProxyResponse(
    ...
    const response = await fetch(`${proxy.url.replace(/\/$/, '')}/chat/completions`, {
      method: 'POST',
      headers: {
  ...
  ): Promise<Partial<GameUpdate>> {
    const prompt = `
      [MỆNH LỆNH TỐI CAO - VARIABLE ACCOUNTANT PROTOCOL]:
      1. BẠN LÀ MỘT KẾ TOÁN DỮ LIỆU (DATA ACCOUNTANT). Nhiệm vụ duy nhất của bạn là ghi chép lại các thay đổi dựa TRỰC TIẾP và DUY NHẤT trên [NỘI DUNG DẪN TRUYỆN VỪA DIỄN RA] và [HƯỚNG DẪN TỪ PROXY 1].
      2. CẤM TUYỆT ĐỐI SÁNG TẠO: Không được thêm bất kỳ tình tiết, nhân vật, vật phẩm, hay sự kiện nào không có trong văn bản dẫn truyện hoặc hướng dẫn.
      3. ƯU TIÊN HƯỚNG DẪN: Nếu Proxy 1 có cung cấp [HƯỚNG DẪN TỪ PROXY 1], hãy ưu tiên thực hiện theo các chỉ dẫn đó để cập nhật biến số.
      4. THỰC TẠI TỐI CAO: Nếu văn bản dẫn truyện viết điều gì đó (dù có thể sai quy tắc hệ thống), bạn PHẢI chấp nhận đó là thực tại và cập nhật biến số theo đó.
      5. CẤM THAY ĐỔI GIÁ TRỊ CŨ: Nếu không có sự thay đổi được nhắc đến, bạn PHẢI giữ nguyên giá trị đó từ [DỮ LIỆU HIỆN TẠI].
      6. TRÍCH XUẤT CHÍNH XÁC:
         - Chỉ số (Stats): Cập nhật máu, vàng, exp...
         - Vật phẩm (Inventory): Thêm/bớt đồ vật.
         - Quan hệ (Relationships): Cập nhật thiện cảm, tâm trạng.
         - Nhiệm vụ (Quests): Cập nhật trạng thái.
      7. GIẢI TRÌNH (evolutionJustification): Giải thích ngắn gọn tại sao bạn cập nhật biến số đó.
      8. CẤM TRẢ VỀ TRƯỜNG "text", "summary" VÀ "suggestedActions".

      [DỮ LIỆU HIỆN TẠI]: ${JSON.stringify(playerObj || {})}
      [HÀNH ĐỘNG CỦA NGƯỜI CHƠI]: ${action}
      [HƯỚNG DẪN TỪ PROXY 1]: ${variableGuidance || "Không có hướng dẫn cụ thể, hãy tự trích xuất từ truyện."}
      [NỘI DUNG DẪN TRUYỆN VỪA DIỄN RA]: ${narrative}
      
      Dựa trên các thông tin trên, hãy cập nhật các biến số hệ thống. Trả về DUY NHẤT một đối tượng JSON theo schema:
      ${GENERAL_JSON_SCHEMA}
    `;

    try {
      const response = await this.getProxyResponse(proxy, prompt, settings, model, 0.1);
      const cleanedText = this.extractValidJson(response);
      if (!cleanedText) return {};
      
      try {
        const fixedJson = this.tryFixJson(cleanedText);
        const parsed = JSON.parse(fixedJson);
        return this.normalizeGameUpdate(parsed);
      } catch (parseError) {
        console.warn("Proxy 2 JSON Parse failed, attempting Tag-based extraction fallback...");
        const statsStr = this.extractTag(response, "statsUpdates");
        const statsUpdates = statsStr ? JSON.parse(this.tryFixJson(statsStr)) : {};
        const evolutionJustification = this.extractTag(response, "evolutionJustification");
        
        return this.normalizeGameUpdate({
          statsUpdates,
          evolutionJustification: evolutionJustification || "Trích xuất dữ liệu từ Proxy 2 bằng phương pháp dự phòng."
        });
      }
    } catch (e) {
    ...
    try {
      ...
      if (normalized.text && playerObj) {
        ...
        if (updates.length > 0) {
          ...
          mvuService.applyUpdates(playerObj.mvuState, updates);
      ...
      if (normalized.text) {
        normalized.text = this.cleanAiContent(normalized.text);
      }
      if (normalized.summary) {
        normalized.summary = this.cleanAiContent(normalized.summary);
      }
  ...
  private cleanHistory(history: any[]): any[] {
    ...
    return history.map(h => ({
      ...
      parts: h.parts.map((p: any) => ({
        ...p,
        text: p.text ? this.cleanAiContent(p.text) : p.text
      }))
    }));
  }
  ...
  public async *getResponseStream(
    ...
    if (settings?.dualProxyEnabled && proxiesToTry.length >= 1) {
      ...
      while (true) {
        ...
        yield { type: 'status', content: 'Đang khởi tạo Dual Proxy (1 AI Viết văn, 1 AI Xử lý biến)...' };

        let fullNarrative = "";
        let writingModel = writingProxy.model || settings?.proxyModel || modelToUse;
        ...
        try {
          ...
          const writingStream = this.getProxyResponseStream(
            ...
            writingProxy.key,
            writingModel,
            finalPrompt + `\n[MỆNH LỆNH TỐI CAO]: Bạn là AI chuyên trách VIẾT VĂN, nhập vai đồng thời là 'Game Master' và 'Tiểu thuyết gia nổi tiếng'. Nhiệm vụ của bạn là dẫn truyện cực kỳ chi tiết, văn phong hoa mỹ, giàu cảm xúc và thú vị. 
            
            QUY TẮC DUAL PROXY:
            1. Bạn KHÔNG cần quan tâm đến việc tính toán biến số phức tạp (máu, vàng, exp...). AI thứ hai sẽ làm việc đó.
            2. Tuy nhiên, bạn PHẢI liệt kê các thay đổi về chỉ số, vật phẩm, quan hệ mà bạn đã mô tả trong truyện vào trường "variableGuidance" trong JSON để hướng dẫn cho AI thứ hai.
            3. Tập trung 100% vào chất lượng văn chương, đối thoại và dẫn dắt tình tiết.
            4. BẮT BUỘC trả về JSON có các trường: "text", "summary", "suggestedActions", "variableGuidance".
            5. Trường "summary" PHẢI là bản tóm tắt đầy đủ, chi tiết bằng TIẾNG VIỆT (VIETNAMESE). Bao quát toàn bộ các tình tiết chính, sự thay đổi cảm xúc của nhân vật và các sự kiện thế giới vừa diễn ra. KHÔNG giới hạn số câu, hãy đảm bảo không bỏ sót bất kỳ ý quan trọng nào vì đây là "bộ nhớ" dài hạn của hệ thống. TUYỆT ĐỐI KHÔNG bao gồm các thẻ <thinking> hay nội dung suy luận kỹ thuật vào đây.
            6. BẮT BUỘC tuân thủ 'CHAT BUBBLE PROTOCOL': Đối thoại phải có dạng [Tên]: "Nội dung". 
            7. Áp dụng 'LITERARY EXCELLENCE PROTOCOL' để nâng cao chất lượng văn chương. 
            8. TUÂN THỦ TUYỆT ĐỐI Ngôi Kể: ${settings?.narrativePerspective || 'Ngôi thứ ba'}.
            9. TUÂN THỦ TUYỆT ĐỐI Văn Phong: ${settings?.writingStyle || 'Mặc định'}.
            10. TUÂN THỦ TUYỆT ĐỐI Độ Dài: ${settings?.responseLength || ResponseLength.WORDS_1000}.
            11. TUÂN THỦ TUYỆT ĐỐI Độ Khó: ${settings?.difficulty || 'medium'}.
            12. TUÂN THỦ TUYỆT ĐỐI Gợi Ý Hành Động: 3-6 gợi ý chi tiết, dài, có thể là chuỗi hành động.`,
            history,
            action,
            settings?.temperature !== undefined ? settings.temperature : 1.0, // Writing AI can be creative (1.0)
            playerObj
          );
          ...
          for await (const chunk of writingStream) {
            if (chunk.type === 'text') {
              ...
              writingData = chunk.content as GameUpdate;
            }
          }

          // MVU: Parse updates from Writing AI narrative
          if (fullNarrative && playerObj) {
            const writingMvuUpdates = mvuService.parseUpdates(fullNarrative);
            if (writingMvuUpdates.length > 0) {
              if (!playerObj.mvuState) playerObj.mvuState = {};
              mvuService.applyUpdates(playerObj.mvuState, writingMvuUpdates);
              // Store these updates to merge later if needed
              if (writingData) {
                writingData.mvuUpdates = writingMvuUpdates;
              }
            }
          }

          // Phase 2: Variable AI
          yield { type: 'status', content: 'AI thứ hai đang xử lý các biến số...' };
          
          const variableModel = variableProxy.model || settings?.proxyModel || modelToUse;
          const variableUpdate = await this.getVariableUpdateFromProxy(
            variableProxy,
            fullNarrative,
            action,
            playerObj,
          ...
          const finalData: GameUpdate = {
            ...
            usedModel: `${writingModel} + ${variableModel}`
          };
          ...
          console.error("Dual Proxy failed:", err);
          const lastError = this.diagnoseError(err, false);
          
          if (retryInfinite) {
            yield { type: 'status', content: 'Dual Proxy lỗi. Đang thử lại ngầm sau 3s...' };
          ...
          if (this.onProxyError) {
            ...
            if (decision === 'cancel') {
              throw new Error(`DUAL_PROXY_FAILED: ${lastError}`);
            }
            if (decision === 'retry_infinite') {
              retryInfinite = true;
            }
            // If retry_once, it just continues the loop
            continue;
          } else {
            yield { type: 'status', content: 'Dual Proxy thất bại, đang thử lại sau 2s...' };
...
3. CẤU TRÚC PHẢN HỒI:
   ...
   - [NỘI DUNG DẪN TRUYỆN CỰC KỲ CHI TIẾT VÀ DÀI]
...
4. LƯU Ý: Khối JSON PHẢI nằm ở CUỐI CÙNG của phản hồi và chứa đầy đủ các trường dữ liệu cần thiết.`,
        ...
        try {
          ...
          if (parsed.text) {
            parsed.text = this.cleanAiContent(parsed.text);
          }
          if (parsed.summary) {
            parsed.summary = this.cleanAiContent(parsed.summary);
          }
  ...
  private async buildPrompt(
    ...
    if (def(ctx?.includePlayerStats)) {
      ...
      compressedMc.attrs = this.safeMapJoin(playerObj?.backgroundAttributes, (a: any) => `${a.label}:${a.value}`, '|');
      compressedMc.cond = this.safeMapJoin(playerObj?.conditions, (c: any) => c.name, ',');
    }

    if (def(ctx?.includePlayerInventory)) {
      compressedMc.inv = this.safeMapJoin(playerObj?.inventory, (i: any) => i.name, ',');
      compressedMc.assets = this.safeMapJoin(playerObj?.assets, (a: any) => a.name, ',');
    }

    if (def(ctx?.includePlayerSkills)) {
      compressedMc.skills = this.safeMapJoin(playerObj?.skills, (s: any) => s.name, ',');
      compressedMc.sSum = playerObj?.skillsSummary;
    }

    if (def(ctx?.includePlayerIdentities)) {
      compressedMc.ids = this.safeMapJoin(playerObj?.identities, (id: any) => id.name, ',');
    }
  ...
  public async suggestGameTime(genre: string, era: string, settings: AppSettings): Promise<Partial<GameTime>> {
    ...
    try {
      return JSON.parse(this.extractValidJson(response));
    } catch (e) {
      return {};
    }
  }
  ...
  public async generateFullWorld(concept: string, model: string, settings: AppSettings, existingData: any): Promise<any> {
    ...
    const response = await this.generateContent(prompt, "Bạn là một kiến trúc sư thế giới RPG chuyên nghiệp.", model, settings);
    return JSON.parse(this.extractValidJson(response));
  }
