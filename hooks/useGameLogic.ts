...
import { GameLog, GameArchetype, Player, SubScenario, Relationship, GameTime, GameGenre, AppSettings, AiModel, CodexEntry, getGenreMeta, ThinkingLevel, StudioParams, WritingStyle, NarrativePerspective, GalleryImage, WorldData, ResponseLength } from '../types';
...
export const useGameLogic = () => {
  ...
  const [modals, setModals] = useState({
    ...
    importWorld: false, proxyRetry: false
  });
  ...
  const handleCommand = useCallback(async (command: string, timeCost?: number, overrideState?: { player: Player, logs: GameLog[], gameTime: GameTime, memory: any }) => {
    ...
    try {
      ...
      if (update.mvuUpdates && update.mvuUpdates.length > 0) {
        const mvuLog = update.mvuUpdates.map(u => `[ MVU ] ${u.path}: ${u.oldValue !== undefined ? `${u.oldValue} -> ` : ''}${u.newValue}`).join('\n');
        currentLogs.push({
          content: mvuLog,
          type: 'system',
          timestamp: Date.now()
