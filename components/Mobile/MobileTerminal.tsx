...
export const MobileTerminal: React.FC<MobileTerminalProps> = ({ 
  logs, onCommand, onOpenAiHint, onRetry, onRegenerateImage, onGenerateSuggestions, onToggleDiagnostics, isLoading, placeholder = "Gõ hành động...", 
      ...
      <div className="space-y-3">
        {groups.map((group, gIdx) => {
          if (group.specialType === 'dialogue') {
            ...
            let bubbleStyles = 'bg-cyan-500/20 border border-cyan-500/50 text-cyan-50 shadow-lg rounded-tl-none';

            if (isMC) {
              nameColor = 'text-amber-400';
              dotColor = 'bg-amber-500 shadow-[0_0_6px_#f59e0b]';
              bubbleStyles = 'bg-amber-500/20 border border-amber-500/50 text-amber-50 shadow-lg rounded-tr-none';
            } else if (isSystem) {
              nameColor = 'text-emerald-400';
              dotColor = 'bg-emerald-500 shadow-[0_0_6px_#10b981]';
              bubbleStyles = 'bg-emerald-500/20 border border-emerald-500/50 text-emerald-50 shadow-lg rounded-tl-none';
            }
            
            return (
              <div key={gIdx} className={`flex ${isMC ? 'justify-end' : 'justify-start'} my-3 animate-in fade-in slide-in-from-${isMC ? 'right' : 'left'}-2 duration-300`}>
                <div className={`max-w-[88%] space-y-1.5`}>
                  ...
                  <div className={`px-4 py-2.5 rounded-2xl transition-all duration-300 ${bubbleStyles}`}>
                    <p className={`leading-relaxed`} style={logStyle}>
          ...
          if (group.specialType === 'thought') {
            return (
              <div key={gIdx} className="my-2 px-3 py-2 bg-purple-500/5 border-l-2 border-purple-500/30 italic text-purple-200/80 animate-in fade-in duration-500 rounded-r-lg">
                <p className={`leading-relaxed`} style={logStyle}>{group.thoughtText}</p>
              </div>
            );
          }
          ...
          if (group.specialType === 'sound') {
            return (
              <div key={gIdx} className="my-2 px-3 py-2 bg-amber-500/5 border-l-2 border-amber-500/30 italic text-amber-200/80 animate-in fade-in duration-500 rounded-r-lg">
                <p className={`leading-relaxed tracking-widest`} style={logStyle}>{group.soundText}</p>
              </div>
            );
          }
          ...
          if (group.specialType === 'system') {
            ...
            let renderedContent = <span className="tracking-tight">{fullContent}</span>;
            ...
            return (
              <div key={gIdx} className={`my-3 animate-in fade-in slide-in-from-bottom-2 duration-500`}>
                <div className={`p-3 rounded-xl border ${themeClasses}`} style={logStyle}>
                  {renderedContent}
                </div>
              </div>
            );
                      ...
                      <React.Fragment key={lIdx}>
                        {parts.map((part, pIdx) => {
                          ...
                          if (pIdx === 0 && trimmedPart.startsWith('GM:')) {
                            return (
                              <span key={pIdx} className="inline-flex items-center gap-1 px-1 py-0.5 bg-cyan-500/20 border border-cyan-500/40 rounded-sm text-cyan-400 mr-1.5" style={logStyle}>
                                GM
                              </span>
                            );
                          }
                          ...
                          if (isSystemBlock) {
                            return (
                              <span key={pIdx} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-emerald-500/10 border border-emerald-500/30 rounded-sm text-emerald-300 mr-1 my-0.5" style={logStyle}>
                                {part.replace(/[\[\]]/g, '').trim()}
                              </span>
                            );
                          }
                          return <span key={pIdx} className={`whitespace-pre-wrap leading-relaxed`} style={logStyle}>{part}</span>;
                        })}
            ...
            <div key={gIdx} className="my-1 bg-emerald-500/5 border border-emerald-500/20 rounded-sm p-2 space-y-1 animate-in fade-in slide-in-from-left-2 duration-300 relative overflow-hidden">
              ...
              {group.isGM && (
                <div className="flex items-center gap-1.5 mb-1.5 border-b border-emerald-500/10 pb-0.5">
                  <span className="px-1 py-0.5 bg-cyan-500/20 border border-cyan-500/40 rounded-sm text-cyan-400" style={logStyle}>GM</span>
                  ...
                  <div key={lIdx} className="flex flex-wrap items-center gap-1">
                    {parts.map((part, pIdx) => {
                      ...
                      if (isSystemBlock) {
                        ...
                        return (
                          <span key={pIdx} className={`inline-flex items-center gap-1 px-1 py-0.5 rounded-sm ${isNpcName ? 'bg-emerald-500/20 border border-emerald-500/40 text-emerald-200' : 'bg-white/5 border border-white/10 text-emerald-400/80'}`} style={logStyle}>
                            {blockText}
                          </span>
                        );
                      }
                      return <span key={pIdx} className="text-neutral-500 italic" style={logStyle}>{part}</span>;
                    })}
                  </div>
                );
              })}
          ...
          if (log.type === 'system') {
            return (
              <div key={actualIndex} ref={isLast ? lastLogRef : null} className="my-1 px-2 py-1 bg-cyan-500/5 border-l border-cyan-500/30 rounded-r-sm">
                <div className="text-cyan-400/80 uppercase tracking-tight italic" style={logStyle}>
            ...
            <div key={actualIndex} ref={isLast ? lastLogRef : (isLastPlayer ? currentTurnRef : null)} className={`animate-in fade-in slide-in-from-bottom-1 duration-300 ${log.type === 'player' ? 'text-emerald-400' : log.type === 'error' ? 'text-rose-400' : 'text-neutral-300'}`}>
              <div className="flex gap-2">
                ...
                <div className="flex-grow flex flex-col min-w-0">
                  <div className={`leading-relaxed relative group/log`} style={logStyle}>
                    ...
                    <div className="mt-3 mb-2 flex flex-wrap gap-2">
                       <button 
                        ...
                        {onGenerateSuggestions && (
                          ...
                          >
                            <Sparkles className={`w-3 h-3 ${isLoading ? 'animate-pulse' : ''}`} />
                            <span className="font-bold" style={logStyle}>
                    ...
                    <div className="space-y-1.5 my-2">
                      ...
                      {log.imageMetadata && (
                        <div className="flex flex-wrap items-center gap-2 px-2 py-1 bg-black/40 border border-white/5 rounded-lg">
                          <span className="text-emerald-500/60" style={logStyle}>[ SYNC_OK ]</span>
                          <span className="text-neutral-600" style={logStyle}>{log.imageMetadata.duration}s</span>
                          <span className="text-neutral-600" style={logStyle}>{log.imageMetadata.model}</span>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {log.type === 'narrator' && log.suggestedActions && log.suggestedActions.length > 0 && actualIndex === lastNarratorIndex && (
                    <div className="flex flex-col gap-2 mt-4">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-emerald-500/40 font-bold" style={logStyle}>Gợi ý hành động</span>
                      </div>
